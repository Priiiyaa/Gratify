...
exports.getFoods = async (req, res) => {
  try {
    const foods = await Food.find()
      .populate('user') // Populate main post user
      .populate({
        path: 'comments',
        populate: {
          path: 'user',
          select: 'profileImage' // Only populate profile image for comment users
        }
      });
...
exports.getFoodById = async (req, res) => {
  try {
    const food = await Food.findById(req.params.id)
      .populate('user', 'name email profileImage') // Populate main user
      .populate({
        path: 'comments', 
        populate: { path: 'user', select: 'name email profileImage' } // Populate user in comments
      });
